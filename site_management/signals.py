...
def site_post_delete_handler(sender, instance, **kwargs):
    ...
    logger.info("##############################")
    logger.info(f"# CADDY DELETE SYNC TRIGGERED: {instance.host}")
    logger.info("##############################")
...
def handle_caddy_sync_required(sender, site, action, **kwargs):
    ...
    # Implement automatic Caddy sync
    try:
        ...
        if action == 'update':
            # Get addresses for the site
            addresses = list(site.addresses.all().values_list('ip_address', flat=True))

            # If no addresses, skip sync
            if not addresses:
                logger.warning(f"No addresses configured for site {site.host}, skipping Caddy sync")
                return

            # Create Caddy config
            config = CaddyConfig(
                host=site.host,
                addresses=[f"{site.protocol}://{addr}" for addr in addresses],
            ...
            # Add/update site in Caddy
            result = caddy.add_site(config)

            if result.get('success'):
                logger.info("##############################")
                logger.info(f"# CADDY AUTO-SYNC SUCCESS: {site.host}")
                logger.info("##############################")
            else:
                logger.error("##############################")
                logger.error(f"# CADDY AUTO-SYNC FAILED: {site.host}")
            ...
            if result.get('success'):
                logger.info("##############################")
                logger.info(f"# CADDY AUTO-DELETE SUCCESS: {site.host}")
                logger.info("##############################")
            else:
                logger.error("##############################")
                logger.error(f"# CADDY AUTO-DELETE FAILED: {site.host}")
                logger.error(f"# ERROR: {result.get('error')}")
                logger.error("##############################")

        elif action == 'update_ssl':
            # Same as update for SSL changes
            addresses = list(site.addresses.all().values_list('ip_address', flat=True))

            if not addresses:
                logger.warning(f"No addresses configured for site {site.host}, skipping Caddy SSL sync")
                return
            ...
            if result.get('success'):
                logger.info("##############################")
                logger.info(f"# CADDY SSL UPDATE SUCCESS: {site.host}")
                logger.info("##############################")
            else:
                logger.error("##############################")
                logger.error(f"# CADDY SSL UPDATE FAILED: {site.host}")
                logger.error(f"# ERROR: {result.get('error')}")
                logger.error("##############################")

    except ImportError as e:
        logger.error(f"Failed to import Caddy manager: {str(e)}")
        logger.error("Automatic Caddy sync disabled - check your installation")
    except Exception as e:
        logger.error("##############################")
        logger.error(f"# CADDY AUTO-SYNC EXCEPTION: {site.host}")
        logger.error(f"# ERROR: {str(e)}")
        logger.error("##############################")
        # Don't raise - signal should not fail the main operation
