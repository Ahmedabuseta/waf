...
from site_mangement.caddy_logger import caddy_logger
...
class EnhancedCaddyManager:
    ...
    def _determine_ssl_strategy(self, config: CaddyConfig) -> Dict:
        ...
        try:
            ...
            # Check for wildcard certificate in storage
            parent_domain = self._get_parent_domain(config.host)
            ...
            if wildcard_cert.exists() and wildcard_key.exists():
                coverage = self.cert_checker.check_domain_coverage(
                    config.host, str(wildcard_cert)
                )

                if coverage.get('matches'):
                    cert_info = self.cert_checker.check_certificate_domains(str(wildcard_cert))
                    ...
                    if self.logger:
                        self.logger.log_certificate_operation(
                            config.host, "wildcard_cert_detected", cert_info
                        )
                    return strategy

            # Check for individual domain certificate
            domain_cert = self.certs_dir / config.host / "cert.pem"
            domain_key = self.certs_dir / config.host / "key.pem"

            if domain_cert.exists() and domain_key.exists():
                coverage = self.cert_checker.check_domain_coverage(
                    config.host, str(domain_cert)
                )

                if coverage.get('matches'):
                    cert_info = self.cert_checker.check_certificate_domains(str(domain_cert))
                    ...
                    if self.logger:
                        self.logger.log_certificate_operation(
                            config.host, "individual_cert_detected", cert_info
                        )
                    return strategy
    ...
    def _apply_ssl_strategy(self, config: CaddyConfig, strategy: Dict):
        ...
        if strategy_type in ["wildcard", "individual", "manual"]:
